...
//getType() means a number instead a color
bool Map::isDeadMap()
{
	...
	for (int i = 1; i < 8; i++){
		for (int j = 0; j < 9; j++)
		{
			...
			if (getMaplists()[i][j].getType() == getMaplists()[i][j + 1].getType()){
				if ((getMaplists()[i][j].getType() == getMaplists()[i - 1][j - 1].getType()) || (getMaplists()[i][j].getType() == getMaplists()[i + 1][j - 1]) ||//对相邻行检查
					(getMaplists()[i][j].getType() == getMaplists()[i - 1][j + 2]) || (getMaplists()[i][j].getType() == getMaplists()[i + 1][j + 2]))
				{
					return false;
				}
				if (j >= 2)
				{
					if (getMaplists()[i][j].getType() == getMaplists()[i][j - 2])//向前检查
						return false;
				}
				if (j <= 6)
				{
					if (getMaplists()[i][j].getType() == getMaplists()[i][j + 3])//向后检查
						return false;
				}
				j++;
			}
	...
	for (int i = 1; i < 8; i++)
	{
		for (int j = 0; j < 9; j++)
		{
			...
			{
				if ((getMaplists()[j][i].getType() == getMaplists()[j - 1][i - 1].getType()) || (getMaplists()[j][i].getType() == getMaplists()[j - 1][i + 1]) ||//对相邻列检查
					(getMaplists()[j][i].getType() == getMaplists()[j + 2][i - 1]) || (getMaplists()[j][i].getType() == getMaplists()[j + 2][i + 1]))
				{
					return false;
				}
				if (j >= 2)
			...
			if (j <= 6)
			{
				...
				{
					if ((getMaplists()[j][i].getType() == getMaplists()[j + 1][i - 1].getType()) || (getMaplists()[j][i].getType() == getMaplists()[j + 1][i + 1].getType()))
						return false;
				}
			}
		}
	...
	for (int j = 0; j < 8; j++)
	{
		...
		if (j >= 0 && j <= 5)
		{
			...
			{
				if ((getMaplists()[0][j].getType() == getMaplists()[0][j + 3].getType()) || (getMaplists()[0][j].getType() == getMaplists()[1][j + 2]))
					return false;
				else if (j == 1)
				{
		...
		else if (j == 6 || j == 7)
		{
			if (getMaplists()[0][j].getType() == getMaplists()[0][j + 1].getType())
			{
				if ((getMaplists()[0][j].getType() == getMaplists()[0][j - 2].getType()) || (getMaplists()[0][j].getType() == getMaplists()[1][j - 1]))
					return false;
				else if (j == 6)
				{
		...
		if (j >= 0 && j <= 5)
		{
			...
			{
				if ((getMaplists()[8][j].getType() == getMaplists()[8][j + 3].getType()) || (getMaplists()[8][j].getType() == getMaplists()[7][j + 2]))
					return false;
				else if (j == 1)
				{
	...
	for (int j = 0; j < 8; j++)
	{
		...
		if (j >= 0 && j <= 5)
		{
			...
			{
				if ((getMaplists()[j][0].getType() == getMaplists()[j + 3][0].getType()) || (getMaplists()[j][0].getType() == getMaplists()[j + 2][1]))
					return false;
				else if (j == 1)
				{
		...
		else if (j == 6 || j == 7)
		{
			...
			{
				if ((getMaplists()[j][0].getType() == getMaplists()[j - 2][0].getType()) || (getMaplists()[j][0].getType() == getMaplists()[j - 1][1]))
					return false;
				else if (j == 6)
				{
		...
		if (j >= 0 && j <= 5)
		{
			...
			{
				if ((getMaplists()[j][8].getType() == getMaplists()[j + 3][8].getType()) || (getMaplists()[j][8].getType() == getMaplists()[j + 2][7]))
					return false;
				else if (j == 1)
				{
