...
//special Piece
void Map::deal_SpecPiece(int i, int n)
{
	if (maplists[i][n].getSpecType() == 1)
	{
		for (int m = 0; m < 8; m++)
			maplists[i][m].setIsClear(1);
	}
	if (maplists[i][n].getSpecType() == 2)
	{
		for (int m = 0; m < 8; m++)
			maplists[m][n].setIsClear(1);
	}
	if (maplists[i][n].getSpecType() == 3)
	{
		if (i > 0)
			maplists[i - 1][n].setIsClear(1);
		if (i < 8)
			maplists[i + 1][n].setIsClear(1);
		if (n > 0)
			maplists[i][n - 1].setIsClear(1);
		if (n < 8)
			maplists[i][n + 1].setIsClear(1);
		if ((i > 0) && (n > 0))
			maplists[i - 1][n - 1].setIsClear(1);
		if ((i > 0) && (n < 8))
			maplists[i - 1][n + 1].setIsClear(1);
		if ((i < 8) && (n > 0))
			maplists[i + 1][n - 1].setIsClear(1);
	...
	if (maplists[i][n].getSpecType() == 4)
	{
		for (int index_x = 0, index_y = 0; (index_x == 8) && (index_y == 8); index_x++, index_y++)
		{
			if (maplists[i][n].getType() == maplists[index_x][index_y].getType())
			{
				maplists[index_x][index_y].setIsClear(1);
			}
		}
	}
	if (maplists[i][n].getSpecType() == 5)
	{
		maplists[i][n].setIsClear(0);
	}
}