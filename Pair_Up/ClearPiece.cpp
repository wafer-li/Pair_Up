		...
		for (int j = 0; j < 8; j++)
		{
			for (int k = 0;; k++)
			{
				...
				if (j == 8)//防止第8和第9块同色时，继续比较而造成越界
					break;
				if (temporary.maplists[i][j].colorNum == temporary.maplists[i][j + 1].colorNum)//方块检查后j++调整j值
					j++;
				else
	...
	for (int i = 0; i < 9; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			for (int k = 0;; k++)
			{
				...
				if (j == 8)
					break;
				if (temporary.maplists[j][i].colorNum == temporary.maplists[j + 1][i].colorNum)//color 的判断方法有待斟酌
					j++;
				else
					break;
			}
